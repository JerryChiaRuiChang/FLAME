...
```{r, warning=FALSE}
result_bit <- FLAME::FLAME_bit(data = data, holdout = holdout, num_covs = 15, covs_max_list = covs_max_list)
```
...
### compare average treatment effect (ATE) by applying `ATE` function.
```{r}
FLAME::ATE(result_bit) #bit vectors
FLAME::ATE(result_PostgreSQL) #PostgreSQL
FLAME::ATE(result_SQLite) #SQLite
```
...
a. When 8 covariates are used for matching, apply `CATE` function to see details of all matched units, including the size and CATE of each matched group. Note that if no units are matched, `CATE` will return nothing.  
```{r}
...
b. When 8 covariates are used for matching, one matched unit has the following covariate combination. 
...
If we would like to see which the CATE and size of this covariate combination, apply `CATE` function by providing (1) number of covaraites (**num_covs**), (2) names of covariates (**cov_name**), and (3) corresponding values (**cov_val**).

```{r, echo = FALSE}
cov_name = colnames(cov_df)[1:8] 
cov_val = as.integer(as.vector(cov_df[,1:8]))
```
...
c. If we would like to see all matched units of (x2 = 1, x3 = 0, x5 = 1) even when there are way more covariates than (x2, x3, x5), we can still apply `CATE` function to outputall possible matches. The input requirements are the same as (b), but **num_covs** argument should not be specified.
...
d. Apply `CATE_plot` to visualize CATE of all matched groups in boxplot.  
```{r}
...
e. Apply `CATE_AVG` to compute average CATE. Note that average CATE is the weighted average of all matched groups.  
```{r}
...
### Get detailed information of all matched units given specific covariate combination  

The following is a specific covariate combination when eight covariates are used for matching.
```{r, echo=FALSE}
matched_df <- FLAME::CATE(FLAME_object = result_bit, num_covs  = 8)
matched_df[1,]
cov_name <- result_bit[[1]][[8]] 
cov_val <- as.numeric(matched_df[1,1:8])
```

If we would like to see all units given this specific covariate combination, apply the following comand.
```{r}
